...
# 0. No normalization.
nonorm.time <- system.time(cells.nonorm <- top_variance(expression_df = cells, gene_names = genes))
...
write.csv(as.data.frame(times), "breast_norm_times.csv")

# save(list=ls()[grepl("cells.", ls())], file = config$data$normalized$breast)
